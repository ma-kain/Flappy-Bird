...
func animate_gameover_panel():
	...
	var showtime = .5
	yield(_animate_current_score(showtime), "completed")
	
	yield(_animate_best_score(showtime), "completed")
	
	yield(_show_medal(), "completed")
...
func _show_medal():
	var t = get_tree().create_timer(1/1000)
...
func _animate_current_score(duration):
	...
	_animate_as_counting_score(0, current_score, 
...
func _animate_best_score(duration):
	...
	var t = get_tree().create_timer(duration)
	_animate_as_counting_score(last_best_score, new_best_score, 
...
func _animate_as_counting_score(from: int, to: int, label: Label, duration: float):
